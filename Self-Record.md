# Transcript Generator

## Description
This generator uses the grading system that is used at the HKUST.
| Letter Grade | Points |
| --- | --- |
| A+ | 4.3 |
| A | 4 |
| A- | 3.7 |
| B+ | 3.3 |
| B | 3 |
| B- | 2.7 |
| C+ | 2.3 |
| C | 2 |
| C- | 1.7 |
| D | 1 |
| F | 0 |

## Target
- [ ] Learn how to build a standard README file. 
- [x] It displays all informations correctly.  
    - Students' personal information  
        - Name, SID, Year of Study, Admit Date, etc.  
    - Information of each semester  
    - Calculation of TGA and CGA and Cumulative Credits Earned  
- [x] It can generate the formatted output on terminal.
- [x] It can read the record that saved in txt or csv.
- [ ] It can save the record that saved in txt or csv. 

## Advanced Target
- [ ] Use Class instead of struct
- [ ] It can generate pdf file of the output.
- [ ] It can generate a TGA and CGA graphs on the last page of that pdf file.

## Progress
Completed:  read csv file (option 2)
            generate transcript on terminal (option 4)

Working on: modify data from csv file (option 3)

Not start:  input data and create a csv file (option 1)
            delete all dynamic data type (option 5)

## Deadline
17/6

## Information Source
1. https://registry.hkust.edu.hk/transcript-grading
2. https://github.com/RichardLitt/standard-readme