...
string Person::GetAdmit_date() const { return admit_date; }

string Person::GetDepartment() const { return department; }

int Person::GetUst_card_num() const { return ust_card_num; }

void Person::ChangeName(string new_name) { name = new_name; }

void Person::ChangeAdmit_date(string new_admit_date) { admit_date = new_admit_date; }

void Person::ChangeDepartment(string new_department) { department = new_department; }
...
Student::Student(string name, string admit_date, string department, int ust_card_num,
                 int year, float cga, float mcga, string status, Program *program,
    ...
    : Person{name, admit_date, department, ust_card_num},
      ...
      status{status},
      program{program},
      total_num_program{total_num_program} {}

Student::~Student() {
  if (program == nullptr) return;

  while (program != program->last_program()) delete program->last_program();
...
float Student::GetCga() const { return cga; }
float Student::GetMcga() const { return mcga; }

string Student::GetStatus() const { return status; }

Program *Student::GetProgram(int index) const {
  if (total_num_program == 0 || total_num_program <= index) return nullptr;
...
int Student::GetTotal_num_program() const { return total_num_program; }

void Student::change_year(int year) { this->year = year; }
      ...
      switch (ptr_course->GetGrade()[0]) {
        case 'A':
          result += ptr_course->GetCredits() *
                    (grade::A + ptr_course->find_decimal_place_grade());
          break;
        case 'B':
          result += ptr_course->GetCredits() *
                    (grade::B + ptr_course->find_decimal_place_grade());
          break;
        case 'C':
          result += ptr_course->GetCredits() *
                    (grade::C + ptr_course->find_decimal_place_grade());
          break;
        case 'D':
          result += ptr_course->GetCredits();
          break;
...
void Student::change_status(string latest_status) { status = latest_status; }
